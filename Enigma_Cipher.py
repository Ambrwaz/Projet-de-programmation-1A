...
#fonction d'affichage des paramètres du jour
def affichage() :
...
#test unitaire global (attention elle peut prendre quelques dizaines de secondes)
def test_Enigma_Cipher() :
     test_enigma()
     test_triples_R()
     test_egalite()
     test_Sigma()
     print("Cryptage d'Enigma : RAS.")

#fonction de prédiction des réglages du jour, attention elle peut prendre du temps en fonction du nombre de triplés de réglage rotor possibles
def prediction() :
    t1 = time.time()
    triple = triples_R()
    t2 = time.time()
    ...
    return(cle_du_jour,connexion_du_jour)
...
#     print("Le réglage initial du jour est :\n",daily_key[today],"\nLe tableau de connexion du jour est : \n",tabl.perm,"\n")
#
#     cle_du_jour,connexion_du_jour = prediction()
...
#fonction à lancer pour chiffrer et déchiffrer un message
def dechiffrage() :
